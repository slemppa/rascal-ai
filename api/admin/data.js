...
if (supabaseUrl && supabaseServiceKey) {
  ...
  logger.warn(
    "[admin-data] ⚠️ Supabase service role env muuttujat puuttuvat – käytetään RLS-rajoitettua clientia",
  );
}
    ...
    if (req.method === "POST") {
      ...
      if (type === "update-features") {
        if (!user_id || !Array.isArray(features)) {
          return res
            .status(400)
            .json({ error: "user_id and features array required" });
        }
        ...
        if (error) {
          logger.error("[admin-data] Error updating features", {
        ...
        return res
          .status(200)
          .json({ success: true, message: "Features updated successfully" });
      }
      ...
      if (type === "update-platforms") {
        if (!user_id || !Array.isArray(platforms)) {
          return res
            .status(400)
            .json({ error: "user_id and platforms array required" });
        }
        ...
        if (error) {
          logger.error("[admin-data] Error updating platforms", {
            message: error.message,
            code: error.code,
          });
        ...
        return res
          .status(200)
          .json({ success: true, message: "Platforms updated successfully" });
      }

      if (type === "update-onboarding") {
        if (!user_id || typeof req.body.onboarding_completed !== "boolean") {
          return res
            .status(400)
            .json({
              error: "user_id and onboarding_completed (boolean) required",
            });
        }
        ...
        const { error } = await db
          .from("users")
          .update({
            onboarding_completed: req.body.onboarding_completed,
            updated_at: new Date().toISOString(),
          })
          .eq("id", user_id);

        if (error) {
          logger.error("[admin-data] Error updating onboarding status", {
            message: error.message,
            code: error.code,
          });
          return res
    ...
    switch (type) {
      ...
      case "stats":
        ...
        const stats = {
          ...
          publishedContent:
            contentData.data?.filter((c) => c.status === "Published").length ||
            0,
          totalCalls: callLogsData.data?.length || 0,
          answeredCalls:
            callLogsData.data?.filter((c) => c.answered).length || 0,
      ...
      case "integrations":
        ...
        if (
          ...
          mixpostConfig?.mixpost_api_token
        ) {
          try {
            ...
            if (
              ...
              Array.isArray(mixpostResponse.data.data)
            ) {
            ...
            console.error(
              "[admin-data] Error fetching Mixpost accounts:",
              mixpostError,
            );
          }
        }
        ...
        if (savedError) {
          console.error(
            "[admin-data] Error fetching saved social accounts:",
            savedError,
          );
        }
        ...
        if (savedAccounts && savedAccounts.length > 0) {
          const mixpostIds = new Set(
            socialAccounts.map((acc) => acc.mixpost_account_uuid),
          );
          const uniqueSavedAccounts = savedAccounts.filter(
            (acc) => !mixpostIds.has(acc.mixpost_account_uuid),
          );
          ...
          if (allError) {
            logger.error("[admin-data] Error fetching all social accounts", {
        ...
        const { data: secrets, error: secretsError } = await db
          .from("user_secrets")
          .select(
            "id, user_id, secret_type, secret_name, metadata, is_active, created_at, updated_at",
          )
          .eq("user_id", user_id)
          .eq("is_active", true)
          .order("created_at", { ascending: false });
        ...
        if (secretsError) {
          logger.error("[admin-data] Error fetching secrets:", {
      ...
      default:
        return res
          ...
          .json({
            error:
              "Invalid type parameter. Use: users, content, segments, stats, or integrations",
          });
    }
