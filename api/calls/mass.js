...
async function handleContactsBasedMassCall(
  req,
  res,
  {
    contacts,
  ...
  if (callLogs.length === 0) {
    return res
      .status(400)
      .json({ error: "Puhelinnumeroita ei löytynyt kontaktidatasta" });
  }
...
async function handleSheetsBasedMassCall(
  req,
  res,
  {
    sheetUrl,
  ...
  let normalizedCampaignId =
    newCampaignId && typeof newCampaignId === "string" && newCampaignId.trim()
      ? newCampaignId.trim()
      : null;
  logger.log("🔍 Campaign ID debug:", {
    newCampaignId,
    normalizedCampaignId,
    type: typeof newCampaignId,
  });
  ...
  const effectiveDate = isScheduled
    ...
    : today;
  const effectiveTime = isScheduled
    ? normalizeTime(scheduledTime)
    : toHHMM(now);
  ...
  if (normalizedCampaignId) {
    ...
    if (campaignError || !campaignData) {
      console.warn("⚠️ Campaign not found or access denied:", {
        normalizedCampaignId,
  ...
  try {
    ...
    if (csvText === null) {
      if (lastStatus === 403) {
        return res
          ...
          .json({
            error:
              "Pääsy estetty (403) – jaa Google Sheets julkiseksi (Anyone with the link can view).",
          });
      }
      if (lastStatus === 404) {
        return res
          ...
          .json({
            error:
              "Välilehteä ei löytynyt (404). Varmista oikea gid-parametri.",
          });
      }
      if (lastStatus && lastStatus >= 500) {
        return res
          .status(502)
          .json({
            error: "Google palautti virheen (5xx). Yritä hetken kuluttua.",
          });
      }
    ...
    if (lines.length === 0) {
      return res
        .status(400)
        .json({ error: "Google Sheets -tiedosto on tyhjä" });
    }
    ...
    const phoneColumns = headers.filter(
      (header) =>
    ...
    const emailColumns = headers.filter(
      (header) =>
    ...
    if (phoneColumns.length === 0) {
      return res
        ...
        .json({
          error:
            'Puhelinnumerosarakkeita ei löytynyt. Tarkista että tiedostossa on sarake nimeltä "phone", "puhelin", "numero" tai "tel".',
        });
    }
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      // Etsi nimi
      const nameColumns = headers.filter(
        (header) =>
      ...
      if (!name) {
        for (let j = 0; j < values.length; j++) {
          if (
            values[j] &&
            !phoneColumns.includes(headers[j]) &&
            !emailColumns.includes(headers[j])
          ) {
            name = values[j];
            break;
          }
      ...
      if (phoneNumber) {
        ...
        // Tarkista onko numero validi kansainvälinen numero (alkaa +, sisältää 7-15 numeroa)
        const isValid = normalized ? /^\+\d{7,15}$/.test(normalized) : false;
        if (!isValid) {
          errorCount++;
          continue;
        }
        ...
        const callLogEntry = {
          ...
          contact_segment_id:
            contactSegmentId && contactSegmentId.trim()
              ? contactSegmentId.trim()
              : null,
          summary:
            script && script.trim()
              ? `Mass-call: ${script.trim().substring(0, 100)}...`
              : `Mass-call: ${callType}`,
          sms_first: Boolean(sms_first),
    ...
    if (callLogs.length === 0) {
      return res
        .status(400)
        .json({ error: "Puhelinnumeroita ei löytynyt tiedostosta" });
    }
