...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      if (csvText === null) {
        if (lastStatus === 403) {
          return res.status(400).json({
            error:
              'Pääsy estetty (403) – jaa Google Sheets "Anyone with the link can view" -asetuksella tai varmista, että linkki on julkinen.',
          });
        }
        if (lastStatus === 404) {
          return res.status(400).json({
            error:
              "Välilehteä ei löytynyt (404). Tarkista, että URL:ssa oleva gid vastaa olemassa olevaa sheet-välilehteä.",
          });
        }
        if (lastStatus === 400) {
          return res.status(400).json({
            error:
              "Virheellinen CSV-pyyntö (400). Kokeile: (1) käytä “Copy link to this sheet” jolloin URL sisältää gid-parametrin, tai (2) varmista julkinen lukuoikeus (Anyone with the link can view).",
          });
        }
        if (lastStatus && lastStatus >= 500) {
          return res.status(502).json({
            error:
              "Google palasi virheellä (5xx). Yritä uudelleen hetken kuluttua.",
          });
        }
      ...
      const isAbort =
        csvError &&
        (csvError.name === "AbortError" || csvError.code === "ABORT_ERR");
      if (isAbort) {
        return res
          .status(504)
          .json({
            error: "CSV-haku aikakatkaistiin (timeout). Yritä uudelleen.",
          });
      }
