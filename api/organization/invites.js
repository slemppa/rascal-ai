// api/organization/invites.js - Unified Invites API
import { withOrganization } from '../_middleware/with-organization.js'
import { sendToN8N } from '../_lib/n8n-client.js'

/**
 * Unified Organization Invites API Endpoint
 * 
 * POST /api/organization/invites - Lähettää uuden kutsun (body: { email, role })
 * POST /api/organization/invites?action=resend - Lähettää kutsun uudelleen (body: { auth_user_id })
 */
async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'resend') {
      return await resendInvite(req, res, orgId, userRole)
    } else {
      return await sendInvite(req, res, orgId, userRole)
    }
  } catch (error) {
 ...
 * Lähetä uusi kutsu käyttäjälle organisaatioon
 */
async function sendInvite(req, res, orgId, userRole) {
...
/**
 * POST /api/organization/invites?action=resend
 * Lähetä organisaatiokutsu uudelleen
 */
async function resendInvite(req, res, orgId, userRole) {
