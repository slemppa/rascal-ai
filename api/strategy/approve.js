...
const N8N_STRATEGY_APPROVAL_URL =
  process.env.N8N_STRATEGY_APPROVEMENT ||
  process.env.N8N_STRATEGY_ARPPVOMENT ||
  "https://samikiias.app.n8n.cloud/webhook/strategy-approvment";
...
async function handler(req, res) {
  ...
  try {
    ...
    if (strategyError || !strategyData) {
      return res
        .status(404)
        .json({ error: "Strategiaa ei löytynyt annetulla strategy_id:llä" });
    }

    const month = strategyData.month || null;

    // Hae käyttäjän content_language asetus
    ...
    const contentLanguage = userData?.content_language || "Suomi";
    ...
    const webhookPayload = {
      ...
      auth_user_id: req.authUser?.id || user_id,
    ...
    let webhookResponse;
    try {
      webhookResponse = await sendToN8N(
        N8N_STRATEGY_APPROVAL_URL,
        webhookPayload,
      );
    } catch (error) {
