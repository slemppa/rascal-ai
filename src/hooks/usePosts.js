  ...
  return supabaseData.map((item) => {
    ...
    if (item.type === "Carousel") {
      ...
      if (item.segments && item.segments.length > 0) {
        const firstSegment =
          item.segments.find((seg) => seg.slide_no === 1) || item.segments[0];
    ...
    const transformed = {
      ...
      title:
        item.idea ||
        item.caption ||
        (t ? t("posts.statuses.untitled") : "Nimetön"),
      status: status,
      thumbnail: thumbnail,
  ...
  return reelsData.map((item) => {
    ...
    return {
      ...
      title:
        item.Idea ||
        item.caption ||
        (t ? t("posts.statuses.untitledReels") : "Nimetön Reels"),
      status: status,
      thumbnail: item.media_urls?.[0] || null,
      caption: item.caption || "Ei kuvausta",
      type: isAvatar ? "Avatar" : "Reel",
      createdAt:
        item.createdTime || item.created_at
          ? new Date(item.createdTime || item.created_at)
              .toISOString()
              .split("T")[0]
          : null,
      scheduledDate: null,
...
export function usePosts(user, t) {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      if (!userId) {
        throw new Error(
          t ? t("posts.messages.userIdNotFound") : "Käyttäjää ei löytynyt",
        );
      }
      ...
      const contentWithSegments = data.map((contentItem) => {
        if (contentItem.type === "Carousel") {
          const itemSegments = segmentsData.filter(
            (segment) => segment.content_id === contentItem.id,
          );
          return {
            ...contentItem,
      ...
      setError(
        t ? t("posts.messages.dataFetchError") : "Tietojen haku epäonnistui",
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchReelsPosts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/social/reels/list?companyId=${userData.company_id}`,
      );
      if (!response.ok) {
        return;
      }
        ...
        if (post.versions && post.versions.length > 0) {
          ...
          if (firstVersion.content && firstVersion.content.length > 0) {
            ...
            if (firstContent.media && firstContent.media.length > 0) {
              ...
              mediaUrls = firstContent.media
                .map((media) => media.thumb_url || media.url)
                .filter(Boolean);
            }
          }
        }
  ...
  return {
    ...
    transformSupabaseData: useCallback(
      (data) => transformSupabaseData(data, t),
      [t],
    ),
